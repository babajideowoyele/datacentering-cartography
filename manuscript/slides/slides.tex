...
WP3 & Structural   & OpenCorporates, REIT filings; Overture Maps (city2graph) & Ownership + urban graph \\
...
\begin{center}
  \includegraphics[width=0.92\textwidth]{../figures/wp1_eci_divergence}
\end{center}
\vspace{-0.3em}
\begin{center}
  \small AWS corporate ECI \alert{120$\times$} higher than Wikipedia --- imaginary production under zero disclosure pressure.\\
  Equinix WAF-blocked: strategic opacity enacted at the \textit{technical} layer.
\end{center}
\end{frame}
...
\begin{column}{0.48\textwidth}
  ...
  \begin{block}{WP3 — Structural Cartography}
    \begin{itemize}
      ...
      \item Overture Maps + city2graph: urban embeddedness graphs
      \item \alert{Ashburn: 36 power nodes, 1 res.\ zone (exurban)}
    \end{itemize}
  \end{block}
\end{column}
\begin{column}{0.48\textwidth}
...
\begin{center}
  \includegraphics[width=0.88\textwidth]{../figures/wp3_urban_embeddedness}
\end{center}
\vspace{-0.3em}
\begin{center}
  \small Ashburn: dense power grid, \alert{near-zero residential adjacency} --- spatial segregation at the land-use level.\\
  Amsterdam/Dublin: 16--61 residential-adjacency edges --- embedded in the urban fabric governance must negotiate.
\end{center}
\end{frame}
...
\begin{center}
  \includegraphics[width=0.95\textwidth]{../figures/wp3_urban_graphs_map}
\end{center}
\vspace{-0.4em}
\begin{center}
  \small Red = data center \quad Blue = power infra \quad Green = residential \quad Lines = proximity edges
\end{center}
\end{frame}
...
Urban embeddedness      & 36 power / 1 res.\ zone (exurban) & 61 res.\ zones; 16 adj.\ edges (embedded) \\
